defmodule Hubiot.Accounts.UserToken do
  use Ecto.Schema
  import Ecto.Query
  ...
  schema "users_tokens" do
    ...
    belongs_to :user, Hubiot.Accounts.User
  ...
  def build_session_token(user) do
    ...
    {token, %Hubiot.Accounts.UserToken{token: token, context: "session", user_id: user.id}}
  end
  ...
  def token_and_context_query(token, context) do
    from Hubiot.Accounts.UserToken, where: [token: ^token, context: ^context]
  end
  ...
  def user_and_contexts_query(user, :all) do
    from t in Hubiot.Accounts.UserToken, where: t.user_id == ^user.id
  end

  def user_and_contexts_query(user, [_ | _] = contexts) do
    from t in Hubiot.Accounts.UserToken, where: t.user_id == ^user.id and t.context in ^contexts
  end
end
